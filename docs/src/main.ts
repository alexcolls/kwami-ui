        ...
        screens: [
            {
                ...
                expandedWidth: 'min(90vw, 700px)',
                expandedHeight: 'min(90vh, 650px)'
            },
            {
                id: 'config',
...
function initApp(): void {
    ...
    multiScreenComponents.forEach(({ expandable, preview, screens }) => {
        ...
        if (expandableEl) {
            ...
            if (preview) {
                const previewEl = expandableEl.querySelector('.kwami-expandable-preview .kwami-button-bezel');
                if (previewEl) {
