...
export class ColorPicker extends Component<ColorPickerProps> {
    ...
    render(): string {
        ...
        const hasActions = showRandomize || showReset || showCopyToOpposite || this.props.showPipette !== false;
    ...
    private setupEventListeners(): void {
        if (!this.trigger || !this.canvas || !this.brightnessSlider ||
            !this.formatToggle || !this.valueInput || !this.popup) return;
        ...
        switch (this.currentFormat) {
            case 'hex':
                if (this.currentOpacity < 100) {
                    ...
                    return (hslToHex(this.currentHue, this.currentSaturation, this.currentLightness) + alphaHex).toUpperCase();
                }
    ...
    private copyToOppositeTheme(): void {
        ...
        // Dispatch custom event
        this.element?.dispatchEvent(new CustomEvent('copytoopposite', {
